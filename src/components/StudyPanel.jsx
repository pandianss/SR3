...
export default function StudyPanel({ sessionQueue, onSessionComplete, energyMode, setTab, apiKey }) {
  ...
  const handleNextSubStep = () => {
    // Determine bounds based on active study modes
    if (energyMode === "low") {
      // Low Energy skips calculation scenarios & quizzes! Just views Concept and Pillars, then rate
      if (currentSubStep === 0) {
        setCurrentSubStep(1); // Go to Pillars
      } else if (currentSubStep === 1) {
        setCurrentSubStep(4); // Skip to Rating screen
      }
    } else {
      if (currentSubStep < 4) {
      ...
      {energyMode === "focus" && timerActive && (
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: timerCount < 20 ? C.err : C.warn, fontFamily: "monospace" }}>
            Exam Drill Timer: {timerCount}s {timerCount === 0 && "(Time's Up - Submit immediately)"}
          </span>
        </div>
      )}
